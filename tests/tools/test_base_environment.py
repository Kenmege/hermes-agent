...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_not_dollardollar_or_bashpid_only(self):
        """The temp name MUST be unique on old and new bash.

        ``$$`` is shared by ``&``-launched subshells, and macOS bash 3.2 leaves
        ``$BASHPID`` unset, so either PID-only strategy can make concurrent
        writers share a temp file. Same-directory ``mktemp`` avoids both races
        while keeping the final ``mv`` atomic.
        """
        env = _TestableEnv()
        env._snapshot_ready = True
        wrapped = env._wrap_command("echo hi", "/tmp")
        ...
        # PID-only temp forms must be gone.
        assert ".tmp.$$" not in wrapped
        assert ".tmp.'$BASHPID" not in wrapped

    def test_temp_path_static_part_is_quoted_mktemp_template(self):
        """The mktemp template must be shlex-quoted so paths with spaces (and
        Windows/Git-Bash ``C:/Users/...`` shapes) stay one shell word."""
        env = _TestableEnv()
        env._snapshot_ready = True
    ...
    def test_init_session_bootstrap_also_atomic_and_mktemp(self):
        ...
        assert ".tmp.'$BASHPID" not in boot
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH.  Crucially it uses same-directory ``mktemp`` so it
    is unique even on macOS bash 3.2 where ``$BASHPID`` is unset.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        _q = shlex.quote
        snap_tmp_template = _q(snap + ".tmp.XXXXXX")
        # One writer iteration = the exact atomic sequence _wrap_command emits.
        writer = (
            "for i in $(seq 1 80); do "
            "export BIG_$i=$(head -c 600 /dev/zero | tr '\\0' x); "
            f"{{ __hermes_snap_tmp=$(mktemp {snap_tmp_template} 2>/dev/null) && "
            "export -p > \"$__hermes_snap_tmp\" && "
            f"mv -f \"$__hermes_snap_tmp\" {_q(snap)}; }} "
            "2>/dev/null || { [ -n \"${__hermes_snap_tmp:-}\" ] && "
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        script = (
            f"{{ __hermes_snap_tmp={bad_tmp}; export -p > \"$__hermes_snap_tmp\" && "
            f"mv -f \"$__hermes_snap_tmp\" {_q(snap)}; }} "
            "2>/dev/null || { [ -n \"${__hermes_snap_tmp:-}\" ] && "
            "rm -f \"$__hermes_snap_tmp\" 2>/dev/null || true; }"
        )
