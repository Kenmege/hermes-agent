...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # calls run) ``$$`` stays the *parent* shell's PID. ``$BASHPID`` closes
        # that race on modern bash, but macOS still ships bash 3.2 where
        # BASHPID is unset. Use same-directory mktemp instead: it is unique on
        # old and new bash, and ``mv`` remains same-filesystem atomic. The
        # static template goes through ``_quote_shell_path`` (upstream #63113)
        # so Windows/Git-Bash drive letters and spaces stay bash-safe.
        _snap_tmp_template = self._quote_shell_path(self._snapshot_path + ".tmp.XXXXXX")
        bootstrap = (
            f"umask 077\n"
            "set -e\n"
            f"__hermes_snap_tmp=$(mktemp {_snap_tmp_template} 2>/dev/null)\n"
            "trap 'rm -f \"$__hermes_snap_tmp\"' EXIT\n"
            ...
            f"alias -p >> \"$__hermes_snap_tmp\" 2>/dev/null || true\n"
            f"echo 'shopt -s expand_aliases' >> \"$__hermes_snap_tmp\"\n"
            f"echo 'set +e' >> \"$__hermes_snap_tmp\"\n"
            f"echo 'set +u' >> \"$__hermes_snap_tmp\"\n"
            # Publish atomically only if assembly succeeded; otherwise the EXIT
            # trap drops the partial temp rather than leaving it to be sourced.
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # Assemble into a per-writer-unique same-directory temp file, then mv to
        # atomically replace the snapshot so concurrent source() calls never
        # read a truncated/half-written file.  mktemp is deliberately used
        # instead of ``$BASHPID`` because macOS bash 3.2 leaves BASHPID unset.
        # Template quoted via ``_quote_shell_path`` (upstream #63113, Windows-safe).
        _snap_tmp_template = self._quote_shell_path(self._snapshot_path + ".tmp.XXXXXX")
